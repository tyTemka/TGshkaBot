...
public class RemoveNoteCommand implements Command {
	...
	public void execute(TelegramBot bot, Message message, String[] args) {
		...
		try {
        	...
        	StringBuilder response = new StringBuilder("🗑️ Какую заметку хотите удалить?\n");
            ...
            bot.setPendingInputHandler(chatId, (noteName) -> {
        		...
        		try {
					...
					bot.sendMessage(chatId, "⚠️Не удалось удалить заметку. Попробуйте позже.");
				}
            });
            
		} catch (Exception e) {
            bot.sendMessage(chatId, "⚠️Не удалось вывести список заметок. Попробуйте позже.");
		}
	}
}